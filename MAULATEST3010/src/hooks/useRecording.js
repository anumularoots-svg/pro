...
export const useRecording = () => {
  ...
  const getRecordingStreamUrl = useCallback(
    (recordingId, email, userId) => {
      ...
      if (typeof recordingsAPI.getVideoStreamUrl === 'function') {
        return recordingsAPI.getVideoStreamUrl(recordingId, userEmail, userIdParam);
      }
  ...
  // ---- Fetch: Trash ----
  const fetchTrashedRecordings = useCallback(
    async () => {
      try {
        setLoading(true);
        setError(null);
  ...
  const moveToTrash = useCallback(
    async (recordingId, userCredentials = null) => {
      try {
        ...
        const creds =
          userCredentials || { user_id: currentUser.id, email: currentUser.email };
  ...
  const restoreFromTrash = useCallback(
    async (recordingId, userCredentials = null) => {
      try {
        ...
        const response = await recordingsAPI.restoreFromTrash(recordingId, creds);
  ...
  const permanentDelete = useCallback(
    async (recordingId, userCredentials = null) => {
      try {
        ...
        const response = await recordingsAPI.permanentDelete(recordingId, creds);
  ...
  const stopMeetingRecording = useCallback(
    async (meetingId) => {
      try {
        ...
        if (!ok) {
          const msg =
            response?.error || response?.message || 'Unknown stop recording error';
          throw new Error(msg);
        }
  ...
  const updateRecording = useCallback(async (recordingId, updates) => {
    try {
      ...
      const response = await recordingsAPI.updateRecording(recordingId, updates);
