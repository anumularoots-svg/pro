...
export const useCalendar = () => {
  ...
  const { user } = useAuth();
  const API_BASE_URL =
    import.meta.env.VITE_API_BASE_URL || "https://192.168.48.201:8119";
  ...
  const createEvent = async (eventData, progressCallback) => {
    try {
      ...
      // Step 4: Create the meeting with reduced timeout
      const response = await retryRequest(async () => {
        return await apiClient.post(
          `${API_BASE_URL}/api/meetings/calendar-meeting`,
          payload,
          {
            timeout: 60000, // 1 minute should be enough for 20 participants
          }
        );
      }, 2);
      ...
      if (deferred.length > 0) {
        ...
        participantResult = await addParticipantsToMeeting(
          ...
          deferred,
          (progress, message) => {
            const overallProgress = 60 + progress * 0.4; // 60% to 100%
            if (progressCallback) {
      ...
      const newEvent = {
        ...
        attendees: participantEmails, // Include all participants in local state
      ...
      if (error.code === "ECONNABORTED") {
        ...
        errorMessage =
          ...
          "Invalid meeting data. Please check your input.";
