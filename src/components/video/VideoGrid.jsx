...
const getParticipantStreamEnhanced = (participant, localStream, remoteStreams) => {
  ...
  // CRITICAL FIX: Build stream keys with LIVEKIT KEYS FIRST
  let streamKeys = [];
  
  // 🔥 PRIORITY 1: LiveKit's native identifiers (MUST BE FIRST)
  if (participant.sid) {
  ...
  // 🔥 PRIORITY 3: Secondary identifiers
  ...
  // 🔥 PRIORITY 4: Role-based keys (for hosts/co-hosts)
  if (isHost || isCoHost) {
    streamKeys.push(
      `host_${participantId}`,
      ...
      `livekit_${participantId}`,
      `remote_${participantId}`,
    );
    
    // Check for generic host/cohost streams
  ...
  console.log('🔑 Stream keys to try (in order):', streamKeys);
  console.log('📦 Available remote streams:', Array.from(remoteStreams.keys()));
  ...
  for (const [streamKey, stream] of remoteStreams.entries()) {
    ...
    // Match by participant ID patterns
    if (participantIdStr && (
    ...
    // Enhanced host/co-host pattern matching
    if (isHost || isCoHost) {
    ...
    // 🔥 NEW: Match by LiveKit participant prefix patterns
    if (participant.sid && keyStr.includes(participant.sid.toLowerCase())) {
      if (stream instanceof MediaStream && stream.getTracks().length > 0) {
        console.log('✅ Found stream with SID pattern match:', streamKey, 'for:', participant.displayName);
        return stream;
      }
    }
    
    if (participant.identity && keyStr.includes(participant.identity.toLowerCase())) {
      if (stream instanceof MediaStream && stream.getTracks().length > 0) {
        console.log('✅ Found stream with identity pattern match:', streamKey, 'for:', participant.displayName);
        return stream;
      }
    }
  }

  console.log('❌ No stream found for participant:', participant.displayName, {
    triedKeys: streamKeys,
