...
class FaceModel:
    ...
    def extract_embedding(self, image_data):
        ...
        try:
            ...
            if len(faces) > 1:
                logger.warning(f"⚠️ Multiple faces detected ({len(faces)}). Using the most prominent face.")
...
class VerifyFace(APIView):
    """Initial face verification endpoint"""
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.info(f"{'='*80}")
            logger.info(f"🔍 INITIAL VERIFICATION - User ID: {user_id} | Port: {SERVER_PORT}")
            logger.info(f"{'='*80}")
            ...
            try:
                ...
                logger.info(f"{'='*80}")
                logger.info(f"📊 INITIAL VERIFICATION RESULTS:")
                ...
                if allowed:
                    response_data["message"] = "✅ Face verified successfully - Match found"
