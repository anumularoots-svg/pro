...
#     """
#     Main function to update all recurring meetings
...
#                 if next_occurrence:
#                     # Update meeting times for today if applicable
#                     if next_occurrence.get('is_today'):
#                         success = update_meeting_for_today(meeting, next_occurrence)
#                         if success:
#                             updated_count += 1
#                             logging.info(f"Updated meeting {meeting['id']} for today")
...
#         - Meetings updated: {updated_count}
...
#     """Get all active recurring meetings from database"""
#     try:
...
#               AND m.Status NOT IN ('ended', 'deleted', 'cancelled')
#               AND (
...
# def update_meeting_for_today(meeting, next_occurrence):
#     """Update meeting times for today's occurrence"""
#     try:
...
#             with connection.cursor() as cursor:
#                 # Update both tbl_Meetings and tbl_ScheduledMeetings
#                 cursor.execute("""
#                     UPDATE tbl_Meetings 
...
#                 logging.info(f"Updated meeting {meeting_id} times for today")
#                 return True
                
#     except Exception as e:
#         logging.error(f"Error updating meeting {meeting.get('id')} for today: {e}")
#         return False
...
#                 sm.is_recurring, sm.recurrence_type, sm.email,
...
#     """Process and send meeting reminders"""
#     try:
#         current_time = get_current_ist_datetime()
...
#             reminder_times = [15, 5, 1]  # minutes before meeting
...
#     """
#     SIMPLE: Only return True if the current meeting has completely ended
#     """
#     try:
...
#         # CRITICAL: Only update AFTER the meeting has ended
#         meeting_has_ended = current_time > current_end_time
        
#         if meeting_has_ended:
#             logging.info(f"Meeting {meeting.get('id')} has ended, ready for next occurrence update")
#             return True
#         else:
#             # Meeting is still active or hasn't started yet
#             logging.debug(f"Meeting {meeting.get('id')} hasn't ended yet. Current: {current_time}, End: {current_end_time}")
#             return False
