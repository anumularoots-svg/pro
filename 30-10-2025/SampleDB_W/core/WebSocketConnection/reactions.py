...
#     """Add reaction (cache only, no database) - Auto-starts if needed"""
#     try:
#         data = json.loads(request.body)
...
#         # Add reaction (auto-starts meeting if needed)
#         success = cache_reactions_manager.add_reaction(
#             data['meeting_id'],
#             data['user_id'],
#             data['user_name'],
#             emoji,
...
#                 'auto_started': not cache_reactions_manager.is_meeting_active(data['meeting_id'])
#             }, status=200)
#         else:
#             return JsonResponse({
#                 'error': 'Failed to add reaction (possibly rate limited or Redis unavailable)'
#             }, status=429)
